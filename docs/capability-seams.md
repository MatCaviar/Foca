...
| `ctx.blockadeGuard` | `core` | [`blockade`](../packages/metacog/blockade) | - | [`blockade-sim`](../packages/metacog/blockade-sim) | - | Verifies declared writes through configured probes and steers agents off blocked path families; the sim worlds and policy adapter exist for keyless experiments. |
